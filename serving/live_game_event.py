...
class LiveGameClient:
    ...
    def process_data_from_id(self) -> pd.DataFrame:

        data = self.get_data_from_id()
    ...
    def _process_events(self, raw_data: Dict[str, Any]) -> pd.DataFrame:
        ...
        plays['team'] = plays['teamId'].apply(lambda tid: 'home' if tid == home_team_id else
        'away' if tid == away_team_id else
        None)
        if plays['team'].isna().any():
            missing = plays[plays['team'].isna()]['teamId'].unique()
            logger.warning(f"Unknown teamId(s) found: {missing}")
        ...
        final_cols = ['gameId', 'eventId', 'team', 'period', 'timeInPeriod',
